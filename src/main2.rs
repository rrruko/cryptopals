...
fn base64_encode(data: &[u8]) -> String {
    ...
    for triplet in data.chunks(3) {
        match triplet {
            [x] => {
                let bits = x as u32 * 256 * 256;
                let out = [ *table.get((bits >> 18) as usize).unwrap()
            ...
            [a,b,c] => {
                let bits = *triplet.get(0).unwrap_or(&0) as u32 * 256 * 256 +
                       *triplet.get(1).unwrap_or(&0) as u32 * 256 +
                       *triplet.get(2).unwrap_or(&0) as u32;
                let out = [ *table.get((bits >> 18) as usize).expect("Something has gone very wrong.")
                          , *table.get(((bits >> 12) % 64) as usize).unwrap_or(&61)
                          , *table.get(((bits >> 6) % 64) as usize).unwrap_or(&61)
                          , *table.get((bits % 64) as usize).unwrap_or(&61)
                          ];
